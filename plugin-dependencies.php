...
class Plugin_Dependencies {
    ...
    public static $repository_uris; // hold where we think we'll find the plugin  repository(jjr)
    public static $plugin_dependants; // List of plugins impacted by this plugin.
    ...
    public static function init($force = false) {
        ...
        static::$repository_uris = array();
        static::$plugin_dependants = array();
        foreach ($all_plugins as $plugin => $plugin_data) {
...
//                        print "</pre>";
                    ...
                    if (!isset(static::$plugin_dependants[$dep])) {
                        static::$plugin_dependants[$dep] = array();
                    }

                    foreach ($all_plugins as $dependantPluginFile=>$the_plugin) {
                        if ($the_plugin['Title'] == $plugin_data['Title']) {
                            $plugin_data['jDependantPluginFile']=$dependantPluginFile;
                            break;
                        }
                    }
                    static::$plugin_dependants[$dep][] = $plugin_data;
...
class Plugin_Dependencies_UI {
    ...
    public static function admin_print_styles() {
        ...
        <style type="text/css">
            ...
            .dep-list li.unsatisfied {
                color: red
            }

            .dep-list li.unsatisfied_network {
                color: orange
            }

            .dep-list li.satisfied {
                color: green
            }
        </style>
    <?php
...
//        }
        ...
        // What depends upon this (jjr)
        // Which plugins are active (this is stooopid - I'm sure this is already stored somewhere
        $network_wide = false;
        if (true === $network_wide) {
            $jactive_plugins = array_keys(get_site_option('active_sitewide_plugins', array()));
        } else {
            $jactive_plugins = get_option('active_plugins', array());

            if (is_multisite()) {
                $jactive_plugins = array_merge($jactive_plugins, array_keys(get_site_option('active_sitewide_plugins', array())));
            }
        }
        if (isset(Plugin_Dependencies::$plugin_dependants[$plugin_file])) {
            $arrDependantsPretty = array();
//            print __FILE__.__LINE__."<BR> ($plugin_file) <pre>";
//            print_r(Plugin_Dependencies::$plugin_dependants);
//            print_r($unsatisfied_network);
//            print "</pre>";
            foreach (Plugin_Dependencies::$plugin_dependants[$plugin_file] as $slot => $plugin_data) {

                  if (in_array($plugin_data['jDependantPluginFile'],$jactive_plugins)) {//<--- you are seeing if this plugin is active instead of the one that depends upon us.
                      if (in_array($plugin_file,$jactive_plugins)) {// Am I active?  Less urgent if I'm turned off.
                          $color = 'orange';
                      } else {
                          $color = 'gray';
                      }
                    $arrDependantsPretty[] = html('span', array('style' => "color:$color",'title'=>'Warning: This plugin is active and needs you.'),$plugin_data['Title']);
                } else {
                      if (in_array($plugin_file,$jactive_plugins)) {// Am I active?  Less urgent if I'm turned off.
                          $color = 'green';
                      } else {
                      ...
                      $arrDependantsPretty[] = html('span', array('style' => "color:$color",'title'=>'OK: This plugin is not active and probably can be deactivated.'),$plugin_data['Title']);
                }
            }
            $htmlDependants = html('span', array('style' => 'color:gray'), '<br/>Required By:'); //'<br/>Required By: ';
            $txtRequiredBy = implode(', ', $arrDependantsPretty);
            $htmlDependants = $htmlDependants . html('span', array('style' => 'color:black'), $txtRequiredBy);
            $actions['needed_by'] = $htmlDependants;

        } else {
            //$htmlDependants .= 'nothing depends upon '.$plugin_file;
        }
//        print __FILE__.__LINE__."<BR> ($plugin_file) <pre>";
//        print_r(Plugin_Dependencies::$plugin_dependants);
//        print "</pre>";
...
//        $htmlDependants = 'Depended Upon by: '; //(jjr)
//        $arrDependantsPretty = array();
//        if (isset(Plugin_Dependencies::$plugin_dependants[$plugin_file])) {
//            foreach (Plugin_Dependencies::$plugin_dependants[$plugin_file] as $slot=>$plugin_data) {
//                $arrDependantsPretty[] = $plugin_data['Title'];
//            }
//            $htmlDependants .= '<-- '. implode(', ', $arrDependantsPretty);
//        } else {
//            //$htmlDependants .= 'nothing depends upon '.$plugin_file;
//        }
//        print __FILE__.__LINE__."<BR> ($plugin_file) <pre>";
...
//        $htmlDependants = 'Depended Upon by: '; //(jjr)
//        $arrDependantsPretty = array();
//        if (isset(Plugin_Dependencies::$plugin_dependants[$dep])) {
//            foreach (Plugin_Dependencies::$plugin_dependants[$dep] as $slot=>$plugin_data) {
//                $arrDependantsPretty[] = $plugin_data['Title'];
//            }
//            $htmlDependants .= '<-- '. implode(', ', $arrDependantsPretty);
//        } else {
//            $htmlDependants .= 'nothing depends upon '.$dep;
//        }
